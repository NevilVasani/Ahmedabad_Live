...
//                    list.setUserid(cursor.getString(cursor.getColumnIndexOrThrow("USERID")));
//
...
public class WishlistActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        wishlistLists = new ArrayList<>();
        adaptor = new WishlistAdaptor(this, wishlistLists, sp, db);
        recyclerView.setAdapter(adaptor);
    ...
    private void createTablesIfNeeded() {
        db.execSQL("CREATE TABLE IF NOT EXISTS USERS(USERID INTEGER PRIMARY KEY AUTOINCREMENT, USERNAME VARCHAR(100), NAME VARCHAR(100), PHONE BIGINT(10), EMAIL VARCHAR(100), PASSWORD VARCHAR(20), GENDER VARCHAR(6), CITY VARCHAR(20))");
        db.execSQL("CREATE TABLE IF NOT EXISTS CATEGORY(CATEGORYID INTEGER PRIMARY KEY AUTOINCREMENT, NAME VARCHAR(100), IMAGE VARCHAR(100))");
        db.execSQL("CREATE TABLE IF NOT EXISTS SUBCATEGORY(SUBCATEGORYID INTEGER PRIMARY KEY AUTOINCREMENT, CATEGORYID VARCHAR(10), NAME VARCHAR(100), IMAGE VARCHAR(100))");
        db.execSQL("CREATE TABLE IF NOT EXISTS PRODUCTS(PRODUCTSID INTEGER PRIMARY KEY AUTOINCREMENT, SUBCATEGORYID VARCHAR(10), CATEGORYID VARCHAR(10), NAME VARCHAR(100), IMAGE VARCHAR(100), DESCRIPTION TEXT, PRICE VARCHAR(20))");
        db.execSQL("CREATE TABLE IF NOT EXISTS WISHLIST(WISHLISTID INTEGER PRIMARY KEY AUTOINCREMENT, USERID VARCHAR(10), PRODUCTID VARCHAR(10))");
    }
        ...
        if (cursor.getCount() > 0) {
            ...
            while (cursor.moveToNext()) {
                ...
                if (productCursor.moveToNext()) {
                    list.setProductId(productCursor.getString(0));
                    list.setSubCategoryId(productCursor.getString(1));
                    list.setCategoryId(productCursor.getString(2));
                    list.setName(productCursor.getString(3));
                    list.setImage(productCursor.getString(4));
                    list.setDescription(productCursor.getString(5));
                    list.setPrice(productCursor.getString(6));
                }
                productCursor.close();
                wishlistLists.add(list);
            }
